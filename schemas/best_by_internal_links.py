...
class BestByInternalLinksSelect(str, Enum):
...
class BestByInternalLinksRequest(BaseRequest):
    _endpoint: str = PrivateAttr("/v3/site-explorer/best-by-internal-links")
    ...
    # Required fields
    select: List[BestByInternalLinksSelect] = Field(
        ...,
        title="Select",
        description=(
...
@response_for(BestByInternalLinksRequest)
class BestByInternalLinksResponse(BaseResponse):
    canonical_to_target: int | None = Field(
        None,
        title="Canonical To Target",
