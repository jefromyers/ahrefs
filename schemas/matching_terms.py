...
class MatchingTermsSelect(str, Enum):
    cpc = "cpc"
    cps = "cps"
...
class MatchingTermsRequest(BaseRequest):
    _endpoint: str = PrivateAttr("/v3/keywords-explorer/matching-terms")
    ...
    # Required fields
    select: List[MatchingTermsSelect] = Field(
        ...,
        title="Select",
        description=(
...
@response_for(MatchingTermsRequest)
class MatchingTermsResponse(BaseResponse):
    cpc: int | None = Field(
        None,
        title="CPC",
