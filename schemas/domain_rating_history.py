...
class DomainRatingHistoryRequest(BaseRequest):
...
@response_for(DomainRatingHistoryRequest)
class DomainRatingHistoryResponse(BaseResponse):
    date: DateType = Field(..., title="Date", description="Date of the metric.")
    domain_rating: float = Field(
        ...,
        title="Domain Rating",
        description=(
            "The strength of your target page's backlink profile compared "
            "to the other websites in our database on a 100-point logarithmic scale"
        ),
    )
